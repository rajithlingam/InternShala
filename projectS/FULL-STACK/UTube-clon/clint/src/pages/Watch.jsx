...
export default function Watch() {
  ...
  return (
    <div className="container">
      ...
      <div className="meta">
        <h2>{v.title}</h2>
        <div style={{color:'var(--muted)'}}>{v.channel?.channelName} • {v.views} views</div>
        <p>{v.description}</p>
        <div className="row" style={{gap:12}}>
          <button className="btn" onClick={()=>like('up')}>👍 Like {v.likes}</button>
          <button className="btn" onClick={()=>like('down')}>👎 Dislike {v.dislikes}</button>
        </div>
        <h3>Comments</h3>
        {user ? (
        ...
        <div>
          {comments.map(c => (
            <div className="comment" key={c._id}>
              <div className="row"><img src={c.user?.avatar} width="24" height="24" style={{borderRadius:999}}/> <b>{c.user?.username || 'User'}</b></div>
