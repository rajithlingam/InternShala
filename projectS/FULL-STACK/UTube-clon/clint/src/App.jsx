...
function Header({ onToggleSidebar }) {
  ...
  return (
    <div className="header">
      ...
      <Link to="/" className="brand">StreamScape</Link>
      <form onSubmit={onSubmit} style={{flex:1, display:'flex', gap:8, maxWidth:700, margin:'0 auto'}}>
        <div style={{position:'relative', flex:1}}>
          <input className="input" placeholder="Search" value={q} onChange={e=>{setQ(e.target.value); onSuggest(e.target.value)}} />
          {sugg.length>0 && (
            <ul className="suggestions">
              {sugg.map(s=> <li key={s} onClick={()=>{setQ(s); setSugg([]); nav('/?q=' + encodeURIComponent(s))}}>{s}</li>)}
            </ul>
          )}
        </div>
        <button className="btn">Search</button>
      </form>
      {!user ? <Link to="/signin" className="btn primary">Sign in</Link> :
        <div className="row">
